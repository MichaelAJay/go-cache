...
type EnhancedCacheMetrics interface {
	...
	RecordCleanup(provider string, reason CleanupReason, itemCount int, duration time.Duration, tags gometrics.Tags)
	RecordMemoryUsage(provider string, totalSize int64, entryCount int64, tags gometrics.Tags)
	RecordProviderSpecific(provider, metricName string, value float64, tags gometrics.Tags)
}
...
type enhancedCacheMetrics struct {
	core        CacheMetrics
	registry    gometrics.Registry
	globalTags  gometrics.Tags
}

// NewEnhancedCacheMetrics creates a new EnhancedCacheMetrics instance
func NewEnhancedCacheMetrics(registry gometrics.Registry, globalTags gometrics.Tags) EnhancedCacheMetrics {
	if registry == nil {
...
func NewNoopEnhancedCacheMetrics() EnhancedCacheMetrics {
	return &enhancedCacheMetrics{
		...
		registry:   gometrics.NewNoop(),
		globalTags: make(gometrics.Tags),
	}
}

// Registry returns the underlying go-metrics registry
func (e *enhancedCacheMetrics) Registry() gometrics.Registry {
	return e.registry
}

// RecordHit records a cache hit
func (e *enhancedCacheMetrics) RecordHit(provider string, tags gometrics.Tags) {
	mergedTags := e.mergeTags(tags)
	e.core.RecordHit(provider, mergedTags)
}

// RecordMiss records a cache miss
func (e *enhancedCacheMetrics) RecordMiss(provider string, tags gometrics.Tags) {
	mergedTags := e.mergeTags(tags)
	e.core.RecordMiss(provider, mergedTags)
}

// RecordOperation records a cache operation with timing
func (e *enhancedCacheMetrics) RecordOperation(provider, operation, status string, duration time.Duration, tags gometrics.Tags) {
	mergedTags := e.mergeTags(tags)
	e.core.RecordOperation(provider, operation, status, duration, mergedTags)
}

// RecordError records an error during cache operations
func (e *enhancedCacheMetrics) RecordError(provider, operation, errorType, errorCategory string, tags gometrics.Tags) {
	mergedTags := e.mergeTags(tags)
	e.core.RecordError(provider, operation, errorType, errorCategory, mergedTags)
}

// RecordCacheSize records the current cache size
func (e *enhancedCacheMetrics) RecordCacheSize(provider string, size int64, tags gometrics.Tags) {
	mergedTags := e.mergeTags(tags)
	e.core.RecordCacheSize(provider, size, mergedTags)
}

// RecordEntryCount records the current number of entries
func (e *enhancedCacheMetrics) RecordEntryCount(provider string, count int64, tags gometrics.Tags) {
	mergedTags := e.mergeTags(tags)
	e.core.RecordEntryCount(provider, count, mergedTags)
}

// RecordBatchOperation records metrics for batch operations
func (e *enhancedCacheMetrics) RecordBatchOperation(provider, operation string, batchSize int, duration time.Duration, tags gometrics.Tags) {
	mergedTags := e.mergeTags(tags)
...
func (e *enhancedCacheMetrics) RecordIndexOperation(provider, operation, indexName string, duration time.Duration, tags gometrics.Tags) {
	mergedTags := e.mergeTags(tags)
...
func (e *enhancedCacheMetrics) RecordSecurityEvent(provider, eventType, severity string, tags gometrics.Tags) {
	mergedTags := e.mergeTags(tags)
...
func (e *enhancedCacheMetrics) RecordTimingProtection(provider, operation string, actualTime, adjustedTime time.Duration, tags gometrics.Tags) {
	mergedTags := e.mergeTags(tags)
...
func (e *enhancedCacheMetrics) RecordCleanup(provider string, reason CleanupReason, itemCount int, duration time.Duration, tags gometrics.Tags) {
	if tags == nil {
	...
	counter := e.registry.Counter(gometrics.Options{
	...
	timer := e.registry.Timer(gometrics.Options{
...
func (e *enhancedCacheMetrics) RecordMemoryUsage(provider string, totalSize int64, entryCount int64, tags gometrics.Tags) {
	if tags == nil {
		tags = make(gometrics.Tags)
	}
	tags["provider"] = provider
	...
	sizeGauge := e.registry.Gauge(gometrics.Options{
	...
	// Record entry count
	entryGauge := e.registry.Gauge(gometrics.Options{
...
func (e *enhancedCacheMetrics) RecordProviderSpecific(provider, metricName string, value float64, tags gometrics.Tags) {
	if tags == nil {
		tags = make(gometrics.Tags)
	}
	tags["provider"] = provider
	...
	gauge := e.registry.Gauge(gometrics.Options{
...
func (e *enhancedCacheMetrics) mergeTags(tags gometrics.Tags) gometrics.Tags {
	if e.globalTags == nil && tags == nil {
		return nil
	}
	
	merged := make(gometrics.Tags)
