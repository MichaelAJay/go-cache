...
type Cache[T any] interface {
	...
	// If indexing enabled: extracts elementKey and ownerKey from value, updates indexes
	// If indexing disabled: uses elementKey extractor for storage key only
	...
	Get(ctx context.Context, elementKey string) (value T, found bool, err error)
	...
	Delete(ctx context.Context, elementKey string) error

	// Clear removes all entries
	// MUST be goroutine-safe but may temporarily affect other operations
	Clear(ctx context.Context) error

	// Has checks if element key exists without retrieving value
	// MUST be goroutine-safe for concurrent access
	Has(ctx context.Context, elementKey string) bool
	...
	// GetOrSet atomically gets existing value by elementKey or sets new value from loader
	// CRITICAL: loader function MUST be called exactly once per elementKey under contention
	...
	GetOrSet(ctx context.Context, elementKey string, loader func(ctx context.Context) (T, error), ttl time.Duration) (T, error)

	// Update atomically updates existing value or creates new value by elementKey
	// CRITICAL: updater function MUST be called atomically - no lost updates
	// MUST handle the case where elementKey doesn't exist (exists=false, old=zero value of T)
	// MUST be goroutine-safe with no possibility of race conditions
	Update(ctx context.Context, elementKey string, updater func(old T, exists bool) (T, error), ttl time.Duration) (T, error)
	...
	// GetMany retrieves multiple elements by elementKeys in a single operation
	// MUST return map with only found keys, missing keys omitted from result
	// MUST be goroutine-safe for concurrent access
	GetMany(ctx context.Context, elementKeys []string) (map[string]T, error)
	...
	// DeleteMany removes multiple elements by elementKeys
	// MUST be goroutine-safe and idempotent (no errors for missing keys)
	// MUST clean up indexes for all deleted elements
	DeleteMany(ctx context.Context, elementKeys []string) error

	// Conditional operations
	// IMPLEMENTATION REQUIREMENT: Must be atomic checks - no race conditions between
	// existence check and set operation

	// SetIfNotExists atomically sets value only if elementKey doesn't exist
	// Uses extractors to determine elementKey from value
	// MUST return true if value was set, false if elementKey already existed
	...
	// SetIfExists atomically sets value only if elementKey exists
	// Uses extractors to determine elementKey from value
	// MUST return true if value was updated, false if elementKey didn't exist
	// MUST be atomic - no race condition between check and set
	...
	// GetMetadata returns metadata for a cache entry by elementKey
	// MUST be goroutine-safe and return accurate metadata snapshot
	// MUST return nil if elementKey doesn't exist (not an error)
	GetMetadata(ctx context.Context, elementKey string) (*CacheEntryMetadata, error)
...
// GetElementKey extracts the primary storage key (e.g., SessionID from Session)
// GetOwnerKey extracts the owner/grouping key (e.g., UserID from Session)
type IndexExtractor[T any] struct {
	GetElementKey func(T) string // Required: extracts primary storage key
	GetOwnerKey   func(T) string // Optional: enables owner-based indexing if provided
}
