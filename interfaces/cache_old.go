...
// Cache_OLD defines the legacy interface for all cache implementations
// This is being replaced by the new generic Cache[T] interface
type Cache_OLD interface {
	// Basic operations
...
// CacheProvider_OLD defines the legacy interface for cache providers
// This is being replaced by the new generic CacheProvider interface
type CacheProvider_OLD interface {
	// Create creates a new cache instance with the given options
	Create(options *CacheOptions) (Cache_OLD, error)
}
...
// CacheMiddleware_OLD defines a function type for legacy cache middleware
type CacheMiddleware_OLD func(next Cache_OLD) Cache_OLD
